...
| `multiseed_train.py` | 5-seed robustness {42,123,7,2024,99} → 100.00 ± 0.00% (headline result in the manuscript) |
...
5. The `window_ablation.py` and `capacity_compare.py` runs were executed in
   separate sessions. Because training is not bit-deterministic (note 2), the
   1.0 s window result differs between them (99.10% vs 98.13%); both lie within
   the reported seed-to-seed standard deviation.
